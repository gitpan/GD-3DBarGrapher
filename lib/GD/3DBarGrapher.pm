...
sub creategraph {
        ...
        my $image = GD::Image->newTrueColor($conf{imgw},$conf{imgh});
        ...
        if($conf{pbgfill} eq "gradient"){
        	gradientfill($bg,$plotleftedge+1,$ypos+1,$plotleftedge+$dims{plotwidth}-1,$ypos+1,$dims{plotheight}-1);
        	gradientfill($bg,($plotleftedge-$dims{floor})+1,($ypos+$dims{floor}),$plotleftedge-1,$ypos+2,$dims{plotheight}-1);
        }
        else{
        ...
        if($conf{bfacecol} ne "random"){
        	...
        	(%shades) = getshades($conf{$conf{bfacecol}}{R},$conf{$conf{bfacecol}}{G},$conf{$conf{bfacecol}}{B});
        }
        else {
        	...
        	(%shades) = getshades($colour{R},$colour{G},$colour{B});
        }
	...
	{
		# get params
		my ($clr,$fromx,$fromy,$tox,$toy,$height,$column) = @_;
		...
		while ($column ne '' ? ($pixposf > $fromy-$height) : ($pixposf < $fromy+$height)) {
			...
			if($column ne ''){
				...
				my $toytemp = $tox;
				while ($ind eq $clr and $toytemp < $conf{imgh}){
					$toytemp++;
